...
{
	caveNoise.reset(new PerlinNoise(seed + 1, 5, 0.5, 10));
	groundNoise.reset(new PerlinNoise(seed + 2, 5, 0.5, 10));
	typeNoise.reset(new PerlinNoise(seed + 3, 2, 10));
}
...
Block* MapGenerator::getBlock(int x, int y, int layer) const{
	...
	double groundVal = groundNoise->noise2d(x, y) + 0.05 * (y - 15);
	double typeVal = typeNoise->noise2d(x, y);
	if (groundVal < 0) // The sky
		return nullptr;

	if (layer == Layer::front && std::abs(caveVal) < 0.1) // The caves
		return nullptr;
